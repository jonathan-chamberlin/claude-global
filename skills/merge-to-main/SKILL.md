...
/path/to/repo-feature         def5678 [feature-branch]
```
...
- **Save the branch name you want to merge** (e.g., `feature-branch`)
- **Save the worktree path if you're in one** (e.g., `repo-feature`)
...
Navigate to the main repository directory (not a worktree folder). Use `git worktree list` output to find the main repo path.
...
Replace `<branch-name>` with the branch you want to merge (e.g., `feature-branch`).
...
**Example:** If the worktree folder was `repo-feature`:
```bash
...
- Always verify you're in the correct directory (main repo, not worktree) before running git commands
